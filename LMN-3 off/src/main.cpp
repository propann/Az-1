...
// ccButtonmatrix columns start at COL_3 (index 0). Adjust if hardware mapping shifts.
const uint8_t CC_COL_SHIFT = 0; // COL_3
const uint8_t CC_COL_PLUS = 4;  // COL_7
const uint8_t CC_COL_MINUS = 5; // COL_8
...
void updatePlusMinus() {
    ...
    if (ccButtonmatrix.getPrevState(CC_COL_SHIFT, 2) == 0) {
        ...
        if (ccButtonmatrix.getPrevState(CC_COL_MINUS, 0) == 0) {
            minusPressed = true;
        } else {
            if (minusPressed) {
